#!/usr/bin/env bash
# run_all_experiments_noperm.sh
# Run run_experiments.sh for every trajectory × antenna-count combination
# with 2 and 3 antennas only, and WITHOUT permutation augmentation on the
# model inputs (--no-permutations passed to train.py).
#
# Trajectory rules:
#   straight  (Experiment_Data_Straight.pkl)  →  2, 3 antennas
#   s_path    (Experiment_Data_S.pkl)          →  2, 3 antennas
#   v_path    (Experiment_Data_V.pkl)          →  2, 3 antennas
#
# Note: phase_relock is a parameter-free baseline and is unaffected by
#       --no-permutations; it is included for completeness.
#
# Usage:
#   ./run_all_experiments_noperm.sh [<base_name>] [OPTIONS]
#
# Arguments:
...
# Only 2 and 3 antennas; no permutation augmentation.
TRAJECTORIES=(
    "straight:Experiment_Data_Straight.pkl:2 3"
    "s_path:Experiment_Data_S.pkl:2 3"
    "v_path:Experiment_Data_V.pkl:2 3"
)
...
for ENTRY in "${TRAJECTORIES[@]}"; do
    ...
    for N_ANT in $ANT_LIST; do
        ...
        echo ""
        echo "╔══════════════════════════════════════════════════════════╗"
        echo "  [$RUN_IDX/$TOTAL_RUNS]  $TRAJ  |  $N_ANT antenna(s)  |  no-permutations"
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "  All experiments complete (no-permutations)"
